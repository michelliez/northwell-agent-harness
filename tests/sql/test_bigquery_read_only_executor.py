...
from google.cloud.exceptions import BadRequest, Forbidden
...
from sql.models import CompiledQuery, PlannedParameter
...
def test_executor_raises_quota_error(
    executor, mock_bigquery_client, mock_job_quota_exceeded
):
...
def test_executor_initialization_with_custom_location():
    ...
    with patch("sql.bigquery_adapter.bigquery.Client"):
        executor = BigQueryReadOnlyExecutor(
            project="test-project", location="eu-london"
        )
...
def test_build_query_parameters_datetime():
    ...
    param = PlannedParameter(
        name="created_at", type="DATETIME", value="2026-01-01 12:00:00"
    )
...
def test_build_query_parameters_timestamp():
    ...
    param = PlannedParameter(
        name="ts", type="TIMESTAMP", value="2026-01-01T12:00:00Z"
    )
...
def test_executor_with_realistic_aggregate_query(
    executor, mock_bigquery_client, mock_job_success
):
...
def test_executor_with_multiple_parameters(
    executor, mock_bigquery_client, mock_job_success
):
