...
from sql.models import ApprovedQueryPlan, PermissionScope
        ...
        for column_name in row.keys():
            ...
            if safety is None:
                ...
                if any(agg.alias.lower() == normalized for agg in approved.plan.aggregations):
                    continue
                if any(grouping.column.lower() == normalized for grouping in approved.plan.groupings):
                    continue
                # Unknown column in result; this is an error
                raise ResultSafetyViolation(
                    f"Result contains unmapped column: {column_name}"
                )
...
def _is_aggregation_alias(name: str, approved: object) -> bool:
    """Check if name matches an aggregation alias."""
    try:
        for agg in approved.plan.aggregations:
            if agg.alias.lower() == name.lower():
                return True
...
def _is_grouping_column(name: str, approved: object) -> bool:
    """Check if name matches a grouping column."""
    try:
        for grouping in approved.plan.groupings:
            if grouping.column.lower() == name.lower():
                return True
    except (AttributeError, TypeError):
        pass
    return False
...
def redact_result(
    ...
    # Build allowed column set from approved plan
    allowed_columns = set()
